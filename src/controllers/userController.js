...
userController.getOneSavedDish = catchAsync(async (req, res, next) => {
  /// saved dish Id is TTV-1 not  SD-1
  const dishId = req.params.dishId;

  let dish = await Dish.findOne({ dishId });
  if (!dish) return next(new AppError("Dish not found", 404));

  const regUser = await SavedRecipe.findOne({ user: req.USER_ID });
  if (!regUser)
    return next(new AppError("Your saved recipes were not found", 404));



  let result;
   if( regUser.savedDishes.includes(dish._id)){result = dish;
  }else{result = "This dish has not been saved"} 
